...
export default function QuoteConfirmationEmail({ name, organization, items, quoteId }: QuoteEmailData) {
  return (
    <Html lang="en">
      <Head />
      <Body style={body}>
        <Container style={header}>
          ...
          <Text style={headerSub}>Quote Request Confirmation</Text>
        </Container>
        ...
        <Container style={container}>
          <Section style={{ textAlign: 'center', paddingBottom: '8px' }}>
            ...
            <Text style={intro}>
              Thank you, <strong>{name}</strong>. Your quote request for <strong>{organization}</strong> has been received
          ...
          <Section>
            <Heading style={h2}>Items You Requested</Heading>
            {items.map((item, i) => (
              <Text key={i} style={itemRow}>
                <span style={{ fontWeight: 700 }}>•</span> {item.product_name}
          ...
          <Section style={timelineBox}>
            ...
            <Text style={timelineItem}>1. Our sales team will review your quote request</Text>
            <Text style={timelineItem}>2. We'll reach out within <strong>1–2 business days</strong> via phone or email</Text>
            <Text style={timelineItem}>3. A customized quotation will be sent to you</Text>
          </Section>
...
const h1: React.CSSProperties = { fontSize: '24px', fontWeight: 800, color: '#0f172a', margin: '0 0 12px', letterSpacing: '-0.5px' };
...
const itemRow: React.CSSProperties = { fontSize: '14px', color: '#374151', margin: '6px 0' };
...
const ctaLink: React.CSSProperties = { fontSize: '14px', color: '#3a7bd5', textDecoration: 'none', fontWeight: 600 };
