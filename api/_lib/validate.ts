...
export const quoteSchema = z.object({
  ...
  phone,
  email,
  organization: z.string().trim().min(2, 'Organization is required').max(255),
  designation: z.string().trim().max(255).optional().default(''),
  facility_type: z.string().trim().max(100).optional().default(''),
...
export const contactSchema = z.object({
  ...
  enquiry_type: z.string().trim().max(100).optional().default(''),
...
export const trainingSchema = z.object({
  ...
  training_category: z.string().trim().max(100).optional().default(''),
  message: z.string().trim().max(2000).optional().default(''),
  _bot_check: botCheck,
});
...
export const careerSchema = z.object({
  ...
  email,
  phone,
  position: z.string().trim().min(2, 'Please select a position').max(255),
  experience: z.string().trim().max(100).optional().default(''),
  cover_letter: z.string().trim().max(10000).optional().default(''),
